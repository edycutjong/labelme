...
- a full card, every NDJSON stream event, the provenance log and the cache keys contain nothing key-shaped (`nsn_…`);
...
- `/api/card` returns **400** for every string that fails `SAFE_QUERY` — 7 named cases plus **10,000 generated
...
6 cards per minute per address (**429** + `Retry-After`), 3,000 live credits per UTC day counted from each card's

...
export function GET(req: NextRequest) {
  ...
  return new ImageResponse(
    <div style={{ width: 1200, height: 630, display: "flex", flexDirection: "column", background: "#0a0e13", color: "#e6edf3", padding: 64, fontFamily: "sans-serif", justifyContent: "space-between" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
        <div style={{ display: "flex", gap: 12 }}>
          {CLASSES.map((c, i) => (
            <div key={c} style={{ padding: "12px 20px", borderRadius: 999, border: "2px solid #2c3b4b", color: i === 0 && score !== null ? "#04150a" : "#b6c2cf", background: i === 0 && score !== null ? "#22c55e" : "transparent", fontSize: 22, fontWeight: 600 }}>
              {c}
            </div>
          ))}
        </div>
        <div style={{ color: "#8b9bab", fontSize: 22 }}>{`${n} recorded wallets · every clue is a Nansen field · labelme.edycu.dev/r/${seed}`}</div>
      </div>
    </div>,

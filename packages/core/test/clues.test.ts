...
describe("extractPnl", () => {
  it("rounds dollars to integers and rates to 3 decimals; keeps up to 5 top tokens", () => {
    const p = extractPnl({ top5_tokens: Array.from({ length: 7 }, (_, i) => ({ token_symbol: `T${i}`, realized_pnl: 10.6, realized_roi: 0.12345 })), traded_token_count: 6, traded_times: 472, realized_pnl_usd: 11892.3, realized_pnl_percent: 0.04123, win_rate: 0.66666 });
  ...
  it("tolerates null top5_tokens and NaN-ish fields", () => {
    const p = extractPnl({ top5_tokens: null, traded_token_count: null, traded_times: undefined, realized_pnl_usd: Number.NaN, realized_pnl_percent: null, win_rate: null });
...
describe("extractTrades", () => {
  it("parses string counts, caps at 5 rows, truncates long symbols", () => {
    const t = extractTrades(Array.from({ length: 6 }, (_, i) => ({ token_symbol: "VERYLONGSYMBOLNAME" + i, pnl_usd_realised: 1.6, nof_buys: "12", nof_sells: 3 })));
...
describe("extractBalance", () => {
  it("token count, total, top share/symbol, stable share; page cap flag from is_last_page:false", () => {
    const b = extractBalance([{ token_symbol: "USDC", value_usd: 300 }, { token_symbol: "PEPE", value_usd: 600 }, { token_symbol: "dust", value_usd: 100 }], false);
  ...
  it("empty balance → zeros with null shares; negative/null values are treated as 0", () => {
    ...
    expect(extractBalance([{ token_symbol: "X", value_usd: -5 }, { token_symbol: "Y", value_usd: null }], true)).toMatchObject({ tokens: 2, totalUsd: 0, topShare: null });
  });
});
...
describe("extractCounterparties", () => {
  ...
  it("falls back to count shares when every volume is null", () => {
    const k = extractCounterparties([{ counterparty_address_label: ["Proxy"], interaction_count: 1 }, { counterparty_address_label: null, interaction_count: 1 }], true);
...
describe("fetchClues — four calls in parallel, sections degrade independently", () => {
  it("a trader personality produces trader-shaped clues from live-shaped responses", async () => {
    ...
    expect(c.calls.map((x) => x.endpoint).sort()).toEqual(["profiler/address/counterparties", "profiler/address/current-balance", "profiler/address/pnl", "profiler/address/pnl-summary"]);
